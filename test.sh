...
HiDENSEC_Code_Folder="/global/scratch/projects/fc_songlab/sanjitsbatra/Rokhsar_Lab_Collaborations/Dirk_Cancer_HiC/HiDENSEC_scripts/"
...
mkdir -p ${outputprefix}".juicebox_tmp_dir.hg38" && java -Xms100g -Xmx100g -jar ${HiDENSEC_Code_Folder}"juicer_tools.jar" pre -t ${outputprefix}".juicebox_tmp_dir.hg38" -n -r 1250000,1000000,500000,250000,100000,50000,25000 ${outputprefix}".mnd.16_fields.txt" ${outputprefix}".hg38.hic" hg38 

# Step-6: Prepare inputs for creating an assembly Hi-C file
cp ${HiDENSEC_Code_Folder}"hg38.cprops" ${outputprefix}.cprops
cp ${HiDENSEC_Code_Folder}"hg38.asm" ${outputprefix}.asm

# Step-7: Create an assembly Hi-C file 
bash ${HiDENSEC_Code_Folder}"3d-dna/visualize/run-asm-visualizer.sh" -q 1 -n -r 1250000,1000000,500000,250000,100000,50000,25000 ${outputprefix}.cprops ${outputprefix}.asm ${outputprefix}".mnd.16_fields.txt"
